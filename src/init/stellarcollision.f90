...
! PURPOSE: To place a stellar model at the centre of a spherical grid and shoot 
!          a point particle into it. The frame is fixed to the centre of the 
!          star.
...
subroutine stellarcollision_rsg
 ...
 integer:: i,j,k,istat,nn,sn,ih1,ihe4
 real(8)::rcore,mcore,dbg,mass,spc_bg(1:spn),radius,imu_const,gradphi
 real(8)::compmass,compsoft,comprad,dis,nsvel,nstheta
...
!-----------------------------------------------------------------------------
 ...
 namelist /crsgcon/ mesafile,spc_list,mass,radius,mcore,rcore,isentropic,&
                    compmass,compsoft,comprad,comp_jet_ang,&
                    dis,nsvel,nstheta

 spc_list='aaa'
! Specify input file, elements you want to track, and a softening length
 open(newunit=nn,file=extrasfile,status='old',iostat=istat)
 if(istat/=0)call error_extras('stellarcollision_rsg',extrasfile)
 read(nn,NML=crsgcon,iostat=istat)
 if(istat/=0)call error_nml('stellarcollision_rsg',extrasfile)
 close(nn)
...
! Set NS motion
 ...
 sink(2)%x(2) = 0d0
 sink(2)%x(3) = 0d0
 sink(2)%v(1) = nsvel*cos(nstheta)
 sink(2)%v(2) = nsvel*sin(nstheta)
 sink(2)%v(3) = 0d0
...
return
end subroutine stellarcollision_rsg
