...
! PURPOSE: Computes the stencil coefficients for radiation
 ...
 subroutine compute_coeffs_radiation(raddim, i, j, k, coeffs)
  ...
  case (11)  ! 1D x-direction
   ...
   if(radswitch == 1) then
    coeffs(1) = coeffs(1) + rad_coupling_term(i,j,k)
   end if
 ...
 function rad_coupling_term(i,j,k) result(couple)
! PURPOSE: Compute radiation coupling term
  use constants,only:clight,arad,Cv
  use grid,only:dt,dvol
  use physval,only:d,T,imu,get_XZ
  use opacity_mod,only:kappa_p

  integer,intent(in):: i,j,k
  real(8):: couple, kappap, X, Z

  call get_XZ(i,j,k,X,Z)
  kappap = kappa_p(X,Z,d(i,j,k),T(i,j,k))
  couple = - dvol(i,j,k)*clight*kappap*d(i,j,k) * &
             ( 4d0*arad*T(i,j,k)**3*kappap*clight*dt / (Cv*imu(i,j,k) &
             + 4d0*arad*kappap*clight*dt*T(i,j,k)**3) - 1d0 )

  return
 end function rad_coupling_term
